...
const Auth = () => {
  ...
  const handleGmailConnect = async () => {
    try {
      setLoading(true)
      // Call PUBLIC social auth endpoint (no auth token needed)
      const response = await fetch(`${API_BASE_URL}/social-auth/gmail/authorize/`)
      const data = await response.json()

      if (!response.ok) {
        throw new Error(data.error || 'Failed to initiate Gmail login')
      }

      // Redirect to Google OAuth consent screen
      window.location.href = data.auth_url
    } catch (err) {
      setError(err.message || 'Failed to connect with Gmail')
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (showForgotPassword) {
        ...
        alert('Password reset link would be sent to: ' + username)
        setShowForgotPassword(false)
        setLoading(false)
        return
      }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 hover:shadow-2xl transition-all duration-700 ease-out">
            ...
            {!showForgotPassword && (
              ...
              >
                ...
                <div className="flex items-center my-6">
                  ...
                  <span className="px-4 text-gray-700 dark:text-gray-400 text-sm">or</span>
                  <div className="flex-1 border-t border-gray-300 dark:border-gray-600"></div>
                </div>
              </motion.div>
            )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <motion.div
              ...
              transition={{ duration: 0.4 }}
            >
              <label className="block text-gray-900 dark:text-white font-medium mb-2">{isSignIn ? 'Username or Email' : 'Username'}</label>
              <input
                type="text"
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                className="input-field"
                placeholder={isSignIn ? "username or email@example.com" : "username"}
                required
              />
            </motion.div>
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-700 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"

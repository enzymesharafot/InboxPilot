...
import { emails, aiService } from '../utils/api'
...
const ComposeEmail = ({ isOpen, onClose, onSend }) => {
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      // Call real Gemini API to compose email based on prompt
      // Using generateReply but with a clear prompt that we want a NEW email composed
      const response = await aiService.generateReply({
        subject: 'Request',
        body: `Compose a professional email with the following request: "${aiPrompt}". Write the complete email including greeting, body, and closing. Do NOT provide instructions on how to write it - write the actual email directly.`,
        sender: 'User',
        tone: 'professional',
        context: `This is a new email composition request. The user wants you to write: ${aiPrompt}`
      })

      const generatedBody = response.reply.body || response.reply
      setBody(generatedBody)

      // If subject is empty, use the generated subject
      if (!subject && response.reply.subject) {
        setSubject(response.reply.subject.replace('Re: ', ''))
      }

      setAiPrompt('')
      setUseAI(false)

      if (onSend) onSend('success', '✨ Email generated by Gemini AI!')
    } catch (error) {
      console.error('AI generation failed:', error)
      if (onSend) onSend('error', `Failed to generate email: ${error.message}`)
    } finally {
          ...
          >
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="border-b border-gray-200 dark:border-gray-700 pb-3">
                  <div className="flex items-center space-x-3">
                    <label className="text-sm font-medium text-gray-800 dark:text-gray-300 w-16">To:</label>
                    <input
                      type="email"
                      value={to}
                ...
                <AnimatePresence>
                  {showCc && (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <label className="text-sm font-medium text-gray-800 dark:text-gray-300 w-16">Cc:</label>
                        <input
                          type="email"
                          value={cc}
                ...
                <AnimatePresence>
                  {showBcc && (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <label className="text-sm font-medium text-gray-800 dark:text-gray-300 w-16">Bcc:</label>
                        <input
                          type="email"
                          value={bcc}
                        ...
                        <button
                          ...
                          className="text-gray-600 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                          </svg>
                ...
                <div className="border-b border-gray-200 dark:border-gray-700 pb-3">
                  <div className="flex items-center space-x-3">
                    <label className="text-sm font-medium text-gray-800 dark:text-gray-300 w-16">Subject:</label>
                    <input
                      type="text"
                      value={subject}
                ...
                {/* Priority Selector */}
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-gray-800 dark:text-gray-300">Priority:</span>
                  <div className="flex space-x-2">
                    <button
                      type="button"
                      onClick={() => setPriority('low')}
                      className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${priority === 'low'
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${priority === 'medium'
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${priority === 'high'
                ...
                <div className="bg-gradient-to-r from-purple-50 via-indigo-50 to-blue-50 dark:from-purple-900/20 dark:via-indigo-900/20 dark:to-blue-900/20 rounded-xl p-4 border border-purple-200 dark:border-purple-700">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${useAI ? 'bg-purple-600 dark:bg-purple-500' : 'bg-gray-300 dark:bg-gray-600'
                        }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${useAI ? 'translate-x-6' : 'translate-x-1'
                          }`}
                      />
                    </button>
                  </div>
                    ...
                    {useAI && (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="w-full bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 dark:from-purple-500 dark:via-indigo-500 dark:to-blue-500 text-white py-2 rounded-lg text-sm font-semibold hover:from-purple-700 hover:via-indigo-700 hover:to-blue-700 dark:hover:from-purple-600 dark:hover:via-indigo-600 dark:hover:to-blue-600 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
                        >
                          {isGenerating ? (
                            <>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-900 flex items-center justify-between">
              <button
                ...
                className="text-sm text-gray-800 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 font-medium flex items-center space-x-1 transition-colors duration-200"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center space-x-3">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-6 py-2 text-gray-800 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg font-semibold transition-all duration-200"
                >
                  Cancel
                </button>
                <button
